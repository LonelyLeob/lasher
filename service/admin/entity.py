...
from utils.fsm import MailingState, SchedulerAddState, SchedulerDeleteState
...
class Admin(object):
    ...
    async def start_add_free_order(self, message: types.Message):
        ...
        await SchedulerAddState.date.set()
    ...
    async def start_delete_order(self, message:types.Message):
        await self.bot.send_message(self.id, "Введите дату, которую хотите удалить")
        await SchedulerDeleteState.date.set()
    
    async def end_delete_order(self, message:types.Message, state:FSMContext):
        await state.finish()
        user_sub = self.schedule.user_is_sub(normalize_to_unix(message.text))
        self.schedule.delete_order(normalize_to_unix(message.text))
        await self.bot.send_message(self.id, f"Запись на {message.text} успешно удалена!")
        if user_sub != 0:
            await self.bot.send_message(user_sub, f"Запись на {message.text} удалена администрацией, приносим свои извинения!")
    ...
    def register_handlers_admin(self, dp:Dispatcher):
        ...
        dp.register_message_handler(self.end_add_free_order, state=SchedulerAddState.date)
        dp.register_callback_query_handler(self.start_delete_order, text='delete')
        dp.register_message_handler(self.end_delete_order, state=SchedulerDeleteState.date)
